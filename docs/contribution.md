...
2、`feat` 分支开发完毕后，本地执行 `pnpm lint` 命令，再执行 `pnpm test` 命令，均通过后向相应人员提 PR，期望合入 `master` 分支，待相应人员 review 代码后合入
...
1、请从 **master** 分支中新建 **fix_xxx** 分支进行修复，如是已发布的版本，分支命名用下划线加上版本号，如：**fix_1.x_xxx**

2、相关问题修复完毕后请向相应人员提 PR，期望合入**master** / x.x_stable 分支，待相应人员 review 代码后合入
